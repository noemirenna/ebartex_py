...
def auction_to_dict(a: Auction) -> dict[str, Any]:
    ...
    return {
        ...
        "game": list(a.game) if a.game else [],
        "card_name": list(a.card_name) if a.card_name else [],
        "condition": list(a.condition) if a.condition else [],
        "images": list(a.images) if a.images else [],
    }
...
# Message when auction is closed but reserve was not reached. Replace with i18n key in multilingua.
RESERVE_NOT_REACHED_MESSAGE_TEMPLATE = "{} hai vinto ma non hai raggiunto il prezzo di riserva"
...
def with_current_status(auction: dict[str, Any]) -> dict[str, Any]:
    """Return auction dict with status recalculated from start_time/end_time."""
...
def with_winner_info(auction: dict[str, Any]) -> dict[str, Any]:
    ...
    if out.get("highest_bidder_id") is not None:
        ...
        if reserve is not None and float(out.get("current_price", 0)) < float(reserve):
            out["reserve_not_reached_message"] = RESERVE_NOT_REACHED_MESSAGE_TEMPLATE.format(
                out["highest_bidder_id"]
            )
    return out
